...
### Score plot: Region colored, Geological structure ellipses

```{r}
#| label: score-plot-region-geo

# 1. Build scores data frame from the PCA result
...
# 3. Explicit color/shape mapping for Region
# NOTE: adjust these names to match your actual unique(scores_df$Region) values
...
ggplot(scores_df, aes(x = PC1, y = PC2 + PC3 + PC4)) +
  ...
  # NOTE: adjust names to match your actual unique(scores_df$Geological.structure) values
  scale_color_manual(values = c("Volanogenic massif" = "blue",
                                 "Cluster II" = "red",
                                 "Hydrogeological basin" = "magenta")) +
...
### Score plot: PC1 vs PC2, colored/shaped by Geological structure

```{r}
#| label: score-plot-pc1-pc2
...
### Score plot: same as above, with "Factor" axis labels

```{r}
#| label: score-plot-factor-labels
